...
void main() {
  ...
  test('backup salvages valid siblings instead of dropping a whole collection', () {
  ...
  test('future backup versions fail closed before any records are accepted', () {
    expect(
  ...
  test('duplicate persistent ids are skipped rather than silently reassigned', () {
  ...
  test('partial current session keeps safe rows and drops only broken children', () {
    final parsed = AppDataStore.parseBackupText(
      jsonEncode({
        'version': 6,
        'currentSession': {
  ...
  test('legacy preferences keep valid rows when one sibling is malformed', () async {
  ...
  test('malformed legacy root falls back to the last coherent auto backup', () async {
  ...
  test('corrupted preference runtime types fall back and clamp safely', () async {
    SharedPreferences.setMockInitialValues({
  ...
  test('legacy to sqlite migration rolls back every table on injected failure', () async {
    ...
    await expectLater(
      store.migrateLegacyData(
        ...
        bodyLogs: [
          BodyLog(id: 'atomic-body', date: DateTime(2026, 9, 5), bodyWeight: 80),
        ],
  ...
  test('partial sqlite without marker is replaced from coherent legacy data', () async {
    final store = LocalSqliteStore(
      factoryOverride: databaseFactoryFfi,
      databasePath: inMemoryDatabasePath,
    );
    addTearDown(store.close);
    await store.replaceSchedules([_schedule('partial-sqlite', 'Wrong partial')]);
    ...
    expect(bundle.schedules.map((e) => e.id), isNot(contains('partial-sqlite')));
  });

  test('corrupt sqlite row falls back to coherent backup without hybrid data', () async {
    final store = LocalSqliteStore(
      factoryOverride: databaseFactoryFfi,
      databasePath: inMemoryDatabasePath,
    );
  ...
  test('wrong backup collection type is isolated while other collections survive', () {
