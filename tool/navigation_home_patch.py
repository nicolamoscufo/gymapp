...
if "import 'ai_coach.dart';" not in text:
    text = text.replace(
...
if start == -1 or end == -1:
    raise RuntimeError('Unable to locate HomePage build tail')
...
new_tail = r'''
  ...
  _PlannedWorkout? _nextPlannedWorkout() {
    final activeSchedules = schedules
        .where((schedule) => !schedule.isArchived && schedule.exercises.isNotEmpty)
        .toList();
...
text = text[:start] + new_tail + text[end + 3:]
home.write_text(text)

shell = Path('lib/screens/home_ai_shell.dart')
shell.write_text(r'''import 'package:flutter/material.dart';
...
void main() {
  ...
  testWidgets('app shell opens dashboard and new primary navigation', (tester) async {
    final now = DateTime.now();
    final schedule = Schedule(
    ...
    await tester.pumpAndSettle();
    expect(find.text('Allenati'), findsOneWidget);
