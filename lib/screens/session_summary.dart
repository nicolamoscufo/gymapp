...
class _SessionSummaryScreenState extends State<SessionSummaryScreen> {
  ...
  double _exerciseVolume(WorkoutExercise exercise) {
    return _completedWorkSets(exercise)
  ...
  Future<void> _openNextSessionPlan() async {
    ...
    try {
      ...
      if (plan == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Scheda originale non trovata: nessuna modifica applicata.',
            ),
          ),
        );
        return;
      ...
      if (!plan.hasActions) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Nessuna modifica alla scheda consigliata per la prossima seduta.',
            ),
          ),
        );
        return;
...
class _NextSessionPlanSheet extends StatefulWidget {
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              ...
                              key: ValueKey(
                                'next-plan-diff-${action.exerciseId}',
                              ),
                              style: theme.textTheme.bodyMedium?.copyWith(
...
class _PostWorkoutDebriefCard extends StatelessWidget {
  final PostWorkoutDebrief debrief;
